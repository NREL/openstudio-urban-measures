...
class Runner
  ...
  def update_measures
    ...
    Open3.popen3(command) do |stdin, stdout, stderr, wait_thr|
      ...
      if wait_thr.value.success?
        ...
        @logger.error(stdout.read.to_s)
        @logger.error(stderr.read.to_s)
      end
    end
  end
    ...
    datapoint_ids.each do |datapoint_id|
      ...
      if existing_datapoint[:datapoint_files]
        existing_datapoint[:datapoint_files].each do |file|
          ...
          begin
            request = RestClient::Resource.new(@url.to_s, user: @user_name, password: @user_pwd)
  ...
  def get_project
    ...
    begin
      request = RestClient::Resource.new(@url.to_s, user: @user_name, password: @user_pwd)
      response = request["/api/project?project_id=#{@project_id}"].get(content_type: :json, accept: :json)
    ...
    if !File.exist?(project_files_dir)
    ...
    result[:project_files].each do |file|
      request = RestClient::Resource.new(@url.to_s, user: @user_name, password: @user_pwd)
      response = request["/api/retrieve_project_file?project_id=#{@project_id}&file_name=#{file[:file_name]}"].get(content_type: :json, accept: :json)
  ...
  def get_all_workflow_ids(feature_type)
    ...
    begin
      request = RestClient::Resource.new(@url.to_s, user: @user_name, password: @user_pwd)
      response = request["/api/workflows?project_id=#{@project_id}"].get(content_type: :json, accept: :json)
  ...
  def get_all_scenario_ids
    ...
    begin
      request = RestClient::Resource.new(@url.to_s, user: @user_name, password: @user_pwd)
      response = request["/api/scenarios?project_id=#{@project_id}"].get(content_type: :json, accept: :json)
  ...
  def get_all_datapoint_ids
    ...
    begin
      request = RestClient::Resource.new(@url.to_s, user: @user_name, password: @user_pwd)
      response = request["/api/datapoints?project_id=#{@project_id}"].get(content_type: :json, accept: :json)
  ...
  def create_osw(datapoint_id)
    ...
    FileUtils.rm_rf(osw_path) if File.exist?(osw_path)
    ...
    all_scenario_ids.each do |scenario_id|
      ...
      scenario[:datapoints].each do |datapoint|
        ...
        if save_datapoint_files
          FileUtils.mkdir_p(detailed_results_dir) if !File.exist?(detailed_results_dir)
      ...
      if !missing_results.empty?
        ...
        if File.exist?(missing_results_path)
      ...
      if File.exist?(summed_result_path)
