...
class CreateTypicalBuildingFromModel < OpenStudio::Measure::ModelMeasure
  ...
  def modeler_description
    return 'This was originally intended for stub space types, but it is possible that it will be run on models that already have internal loads, schedules, or constructions that should be preserved. Set it up to support addition at later date of bool args to skip specific types of model elements.'
  end
  ...
  def run(model, runner, user_arguments)
    ...
    # todo - in future make template in this measure an optional argument and only override value when it is not initialized;
    # todo - there may be valid use cases for using different template values in different measures within the same workflow.
