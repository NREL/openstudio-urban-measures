...
class RadianceMeasure < OpenStudio::Ruleset::ModelUserScript
  ...
  def modelToRadPreprocess(model)
    ...
    model.getSpaces.each do |space|
      ...
      space.surfaces.each do |surface|
        ...
        if (!adjacentSurface.empty?)
          ...
          adjacentSurface.get.subSurfaces.each do |subSurface|
            subSurface.remove 
          end
        end
      end
    ...
    result.getShadingSurfaceGroups.each do |shadingSurfaceGroup|
    ...
    result.getOutputVariables.each do |outputVariable|
  ...
  def run(model, runner, user_arguments)
    ...
    if !got_2x
      ...
      runner.registerInfo("OpenStudio CLI version: #{OpenStudio.openStudioLongVersion.to_s}")
        ...
        windowGroups.each do |wg|
          ...
          windowControls.row(wg_index-1).each_with_index do | illuminance, row_index|
            ...
            if window_illuminance < shadeControlSetpoint
              print_statement("E(#{windowGroup}) is #{window_illuminance.round(0)} lux at index #{row_index}: STATE=0 (up/clear)", runner) if row_index > 149 && row_index < 162  if debug_mode # print shade decisions for one day
              ...
              print_statement("E(#{windowGroup}) is #{window_illuminance.round(0)} lux at index #{row_index}: STATE=1 (dn/tinted)", runner) if row_index > 149 && row_index < 162  if debug_mode # print shade decisions for one day
      ...
      8760.times do |hour|
        ...
        t_space_names_to_calculate.each do |space_name|
          ...
          if values.size > 0
            ...
            print_statement("starting illuminance map for '#{space_name}'. space_size: #{space_size}, index is now at: #{index}, ", runner) if hour == 0 if debug_mode
    ...
    if got_2x
      ...
      sqlPath = OpenStudio::Path.new("../eplus_preprocess/run/eplusout.sql")
    ...
    exec_statement("epw2wea \"#{weather_file_path.to_s}\" wx/in.wea", runner)
    ...
    runner.registerWarning("'Cleanup data' option selected, will delete ancillary Radiance data and all Radiance input files, post-simulation.") if debug_mode && cleanup_data
#
    ...
    if cleanup_data
      runner.registerWarning('Deleting most Radiance I/O to preserve disk space')
      runner.registerWarning('Deleting debug files') if debug_mode
