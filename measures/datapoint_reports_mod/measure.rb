...
######################################################################
##AA: This file is modified to make the reporting compatible with the "replace idf" measure, with references to the open studio model removed. 
  ...
  def run(runner, user_arguments)
    ...
    OpenStudio::EndUseFuelType.getValues.each do |fuel_type|
      ...
      OpenStudio::MonthOfYear.getValues.each do |month|
        if month >= 1 and month <= 12  #AA: gets values from SQL file 
    ...
    # queries that don't have API methods yet #AA: extracting things from SQL file 
	...
	runner.registerInfo("BEGIN_MONTH: #{begin_month}, END_MONTH: #{end_month}")
    ...
    #get aspect ratios ##AA: extracting things from SQL file 
    ...
    timeseries.each_index do |i|
	  ...
	  runner.registerInfo("key_values:#{key_values},for timeseries_name:#{timeseries_name}")
      if key_values.empty?
        key_value = ""
      else
      ...
      if n.nil? 
		...
		runner.registerInfo("N.nil is here")
        values[i] = ts.get.values
        n = values[i].size 
		runner.registerInfo("N is set to: #{n}")
      elsif ts.is_initialized
	    runner.registerInfo("N is initalized")
        values[i] = ts.get.values
		runner.registerInfo("Size of values[i]: #{values[i].size}")
		runner.registerInfo("Class: #{values[i].class}")
		
      else
        #values[i] = Array.new(n, 0)
		values[i] = OpenStudio::Vector.new(n)
		(0..n-1).each do |k|
			values[i][k] = 0
		end 
		runner.registerInfo("N is set to: #{n}")
	  ...
	  runner.registerInfo("values: #{values[i]}")
    end
    ...
    File.open("report.csv", 'w') do |file| 
      ...
      (0...n).each do |i|
		...
		runner.registerInfo("i values: #{i}, j values: #{j}") 
          line << values[j][i]          
    ...
    values.each do |value| 
      ...
      all_values.each do |v|
        day_sum += v
        if i == 24*timesteps_per_hour #AA:uses data from a model. command 
          daily_sums << day_sum
          i = 1
          day_sum = 0
        else
      ...
      monthly_sums = []  
      if begin_month == 1 && begin_day_of_month == 1 && end_month == 12 && end_day_of_month == 31 #AA: commented out b/c uses monthly data accessed thru model. 
