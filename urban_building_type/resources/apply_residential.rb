...
def apply_residential_constructions(model, living_space_type, basement_space_type, runner)
  ...
  require_relative 'beopt-measures/ProcessConstructionsExteriorInsulatedWallsCMU/measure.rb'
  ...
  require_relative 'beopt-measures/ProcessConstructionsFinishedBasement/measure.rb'
  require_relative 'beopt-measures/ProcessConstructionsInsulatedRoof/measure.rb'
  require_relative 'beopt-measures/ProcessConstructionsWindows/measure.rb'
  ...
  when "Single-Family"
	...
	args_hash["selectedliving"] = living_space_type_name
	...
	if basement_space_type_name
		args_hash["selectedfbsmt"] = basement_space_type_name
	end
	run_measure(model, measure, args_hash, runner)
...
def apply_residential_characteristics(model, space_type, runner)
  ...
  require_relative 'beopt-measures/AddResidentialBedroomsAndBathrooms/measure.rb'
...
def apply_residential_appliances(model, space_type, runner)
  ...
  require_relative 'beopt-measures/AddResidentialRefrigerator/measure.rb'
  require_relative 'beopt-measures/ResidentialCookingRange/measure.rb'
...
def apply_residential_lighting(model, living_space_type, basement_space_type, runner)
  ...
  require_relative 'beopt-measures/ResidentialLighting/measure.rb'
  ...
  when "Single-Family"	
	...
	args_hash["space_type"] = living_space_type_name	
	if basement_space_type_name
		args_hash["fb_space_type"] = basement_space_type_name
	end
	run_measure(model, measure, args_hash, runner)
...
def apply_residential_mels(model, living_space_type, runner)

  require_relative 'beopt-measures/ResidentialMiscellaneousElectricLoads/measure.rb'
...
def apply_residential_hvac(model, living_space_type, living_thermal_zone, basement_thermal_zone, runner)
  ...
  require_relative 'beopt-measures/ProcessHeatingandCoolingSetpoints/measure.rb'
  require_relative 'beopt-measures/ProcessElectricBaseboard/measure.rb'
  require_relative 'beopt-measures/ProcessFurnace/measure.rb'
  ...
  when "Single-Family"	
    ...
    args_hash = default_args_hash(model, measure)
    args_hash["selectedliving"] = living_thermal_zone_name
    run_measure(model, measure, args_hash, runner)
    ...
    # if basement_thermal_zone
	  # args_hash["selectedfbsmt"] = basement_thermal_zone_name
    # end	
    # run_measure(model, measure, args_hash, runner)
...
def get_space_types(model)
  ...
  model.getSpaces.each do |space|
    if space.name.to_s.include? "Story 0"
	...
	elsif space.name.to_s.include? "Story 1"
  ...
  model.getSpaces.each do |space|
	next if space.name.to_s.include? "Story 0"
...
def apply_residential(model, runner)
  ...
  result = result && apply_residential_mels(model, living_space_type, runner)
